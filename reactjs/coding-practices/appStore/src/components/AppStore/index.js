...
class AppStore extends Component {
  state = {currentCategory: tabsList[0].tabId, searchInput: ''}

  onClickTab = tabId => {
    this.setState({currentCategory: tabId})
  }
        ...
        <ul className="tabList-container">
          {tabsList.map(eachItem => (
            <TabItem
              tabItem={eachItem}
              key={eachItem.tabId}
              toggleTab={this.onClickTab}
              isActiveTab={eachItem.tabId === currentCategory}
            />
          ))}
        </ul>
        <ul className="appsList-container">
