...
class MatchGame extends Component {
  state = {category: 'FRUIT', randomImgIndex: 0, score: 0, sec: 60}
  ...
  timer = () => {
    this.setState(prevState => ({
      sec: prevState.sec > 0 ? prevState.sec - 1 : 0,
    }))
  }

  changeCategory = updatedCategory => {
    this.setState({category: updatedCategory})
  }

  checkImg = id => {
    const {randomImgIndex} = this.state
    const imgId = imagesList[randomImgIndex].id

    if (id === imgId) {
      const randomImg = Math.floor(Math.random() * 30)
      this.setState(prevState => ({
        score: prevState.score + 1,
        randomImgIndex: randomImg,
      }))
    } else {
      this.setState({sec: 0})
    }
  ...
  onReset = () => {
    this.setState({score: 0, sec: 60})
  }

  render() {
    const {category, score, randomImgIndex, sec} = this.state
    const updatedImagesList = imagesList.filter(
      eachImg => eachImg.category === category,
    )

    const currImg = imagesList[randomImgIndex]
    const {imageUrl} = currImg

    return (
      <>
        <ul>
          <NavBar score={score} sec={sec} />
        </ul>
        {sec === 0 ? (
